...
const ESP32_MIC_SAMPLE_RATE = Number(Deno.env.get("ESP32_MIC_SAMPLE_RATE") ?? "16000");
const MATRIBLOOD_API_URL = Deno.env.get("MATRIBLOOD_API_URL") ?? "http://127.0.0.1:8001";

async function runMatriBloodOptimization(transcript: string) {
	try {
		const eventResponse = await fetch(`${MATRIBLOOD_API_URL}/hardware/voice-optimization`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ transcript }),
		});
		if (!eventResponse.ok) {
			throw new Error(`hardware optimization failed: ${eventResponse.status}`);
		}
		...
		console.log("[MATRIBLOOD-Q] Quantum optimizer triggered from hardware transcript.");
		console.log(
			`[MATRIBLOOD-Q] Qiskit available=${metadata.qiskit_available}, solver=${metadata.solver_type}, variables=${metadata.binary_variables?.length ?? 0}, constraints=${metadata.constraint_count ?? 0}`,
		);
		console.log(
			`[MATRIBLOOD-Q] Complete-kit ETA=${optimized.complete_kit_eta_minutes ?? "not feasible"} min, objective=${optimized.objective_value}`,
		);
		for (const action of optimized.actions ?? []) {
			console.log(
				`[MATRIBLOOD-Q] ${action.priority_order}. ${action.units} x ${action.blood_group ? `${action.blood_group} ` : ""}${action.product_type} from ${action.source_name} via ${action.courier_id ?? "courier pending"} (${action.eta_minutes}m)`,
			);
		}
		return comparison;
	} catch (error) {
		console.error("[MATRIBLOOD-Q] Optimization bridge failed:", error);
		return null;
	}
}
			...
			if (isBinary) {
				...
				if (isElevenLabsConnected && elevenLabsConnection) {
					...
					if (audioChunkCount % 25 === 0) {
						console.log(
							`Sending audio chunk to ElevenLabs: raw=${data.length} bytes, resampled=${pcmForEleven.length} bytes, inRate=${ESP32_MIC_SAMPLE_RATE}, elevenInRate=${elevenInputSampleRate}, level=${audioLevel}`,
						);
					}
			...
			switch (event.type) {
				...
				case "user_transcript":
					if (event.user_transcription_event?.user_transcript) {
						...
						await runMatriBloodOptimization(
							event.user_transcription_event.user_transcript,
						);
					}
					break;
